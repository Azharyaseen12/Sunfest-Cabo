...
export default function SelectYourTicketType({
	...
	const [selectedTicket, setSelectedTicket] = useState(
		bookingData?.selectedTicket || null
	)
	...
	const hasOneDayInventory = (ticket) => {
		return ticket.inventory?.some(
			(item) =>
				item.ticket_type_day === 'ONE_DAY' && item.remaining_inventory > 0
		)
	}
		...
		const fetchInventories = async () => {
			try {
				const ticketTypesResponse = await api.get(
					`event/ticket-types/?package=${packageId}`
				)
					...
					ticketTypes.map(async (ticket) => {
						try {
							const inventoryResponse = await api.get(
								`event/ticket-inventory/?ticket_type=${ticket.id}`
							)
							return {
								...ticket,
							...
							console.error(
								`Error fetching inventory for ticket ${ticket.id}`,
								err
							)
							return {
								...ticket,
												...
												.map((date) => {
													...
													return (
														<Box
															key={date?.id}
															onClick={() =>
																!isDisabled && handleDateSelect(date, ticket.id)
															}
															sx={{
																width: 40,
																height: 40,
